...
class SatelliteTLERequest(BaseModel):
    ...
    tle_line1: str = Field(
        ..., pattern=r"^1 \d{5}[UWC] .*", description="First line of TLE"
    )
    tle_line2: str = Field(
        ..., pattern=r"^2 \d{5} .*", description="Second line of TLE"
    )
...
class SatellitePosition(BaseModel):
    ...
    altitude_km: float = Field(
        ..., gt=0, description="Height above WGS84 ellipsoid in km"
    )
...
class OrbitPath(BaseModel):
    """
    מחזיר רשימה של נקודות המייצגות את מסלול הלוויין.
    """

    sat_name: str
    coordinates: List[SatellitePosition]  # שימוש חוזר במודל הקיים
